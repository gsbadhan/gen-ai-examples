...
# External fallback search using Tavily
fallback_search_tavily = TavilySearch(max_results=1)


# 1. Prepare the LLM and Prompt Template with RAG context
prompt = ChatPromptTemplate.from_messages([
    ("system", "You are a helpful research assistant. Use the context and chat history to answer."),
...
def respond_node(state: dict) -> dict:
    ...
    #  Perform document retrieval (RAG)
    ...
    # Tavily fallback
    fallback_keyords=["no information", "sorry", "not provide", "not include information"]
    if any(keyword in response.content.lower() for keyword in fallback_keyords):
        print(f"fallback to Tavily search...")
        tvly_response = fallback_search_tavily.invoke(input=input_message)
        print(f"tavily api response={tvly_response}")
        response = format_tavily_results(tvly_response)
